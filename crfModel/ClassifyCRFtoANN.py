...
#Work in progress for file feed in
'''
import os

input_root ='./malletformat'

test_folder = 'test'

output_fold ='predictedAnns'


tests = sorted( [os.path.join(input_root, test_folder, test) for test in os.listdir(os.path.join(input_root, test_folder))])
outs = sorted( [os.path.join(output_fold, test[:-4]+'.ann') for test in os.listdir(os.path.join(input_root, test_folder))])

for test, out in zip(tests, outs):
    print(test)
    crf = pickle.load(open("linear-chain-crf.model.pickle", "rb"))
    (test_sents, test_sents_indices) = convertCONLLFormJustExtractionSemEvalPerfile(test)
    ...
    test_sents_pls_phrases = [phrasesFromTestSenJustExtractionWithIndex(x, y) for (x, y) in
                              zip(test_sents_pls, test_sents_indices)]
    i = 0
